...
const get_wishlist = async(idArg) => {
    try {
...
const get_products_from_wishlist = async(wishlist) => {
...
const add_to_wishlist = async(clientIdArg, productIdArg) => {
    try {
        const findItem = await prisma.wishlist.updateMany({
            where: {
                AND: [
        ...
        if(!findItem.count) {
            const createItem = await prisma.wishlist.create({
                data: {
                    amount: 1,
                    clientID: clientIdArg,
                    productID: productIdArg,
...
const delete_from_wishlist = async(clientIdArg, productIdArg) => {
    try {
        const findItem = await prisma.wishlist.updateMany({
            where: {
                AND: [
...
const generators = {
    ...
    getProductsFromWishlist: get_products_from_wishlist,
